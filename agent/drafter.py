...
def evidence_to_text(evidence: list[EvidenceFact]) -> str:
    """
    Summary:
    evidence_to_text turns a list of EvidenceFact objects into readable bullet
    points. This gives the email drafter grounded facts from the data.

    @param evidence: the list of evidence facts selected by rules.py

    @return: a string of evidence facts formatted as bullet points
    """
    evidence_lines = []
...
def get_tone_instruction(outreach_angle: str) -> str:
    """
    Summary:
    get_tone_instruction returns a short tone description based on the outreach
    angle. The LLM uses this tone to make the generated email fit the situation.

    @param outreach_angle: the outreach strategy selected by rules.py

    @return: a short tone description for the email drafter
    """
...
def parse_llm_email(email_text: str) -> EmailDraft:
    """
    Summary:
    parse_llm_email separates the LLM response into a subject and body. The
    prompt asks the LLM to return the email in the format "Subject: ..." and
    "Body: ...".

    @param email_text: the raw email text returned by the LLM

    @return: an EmailDraft containing the parsed subject and body
    """
...
def draft_email_with_llm(
    ...
    The business logic already happened in rules.py, so the LLM should only
    write from the outreach angle and evidence provided here.

    @param account_name: the account name for the email
    @param outreach_angle: the outreach strategy selected by rules.py
    @param evidence: the list of evidence facts selected by rules.py

    @return: an EmailDraft generated by the LLM
    """
    client = OpenAI()
...
def has_value(text: str) -> bool:
    """
    Summary:
    has_value checks whether a CRM field has a real value. This prevents the
    template email from printing missing values like None or null.

    @param text: the text value to check

    @return: True if the text has a usable value, otherwise False
    """
    if text is None:
        return False
...
def make_phrase_lower(text: str) -> str:
    ...
    @param text: the raw next-step text from the CRM data

    @return: a lowercase or natural-sounding version of the phrase
    """
...
def get_opportunity_details(evidence: list[EvidenceFact]) -> dict[str, str]:
    ...
    @return: a dictionary containing opportunity_name, stage, and next_step
    """
    for fact in evidence:
...
def draft_email_with_template(
    ...
    """
    Summary:
    draft_email_with_template creates a basic outreach email without calling an
    LLM. This is the fallback version used when there is no OpenAI API key.

    @param account_name: the account name for the email
    @param outreach_angle: the outreach strategy selected by rules.py
    @param evidence: the list of evidence facts selected by rules.py

    @return: an EmailDraft created from deterministic template logic
    """
    ...
    if outreach_angle == "open opportunity follow-up":
        ...
        if has_value(stage) and has_value(next_step):
        ...
        elif has_value(stage):
            body = (
                f"Hi there,\n\n"
                f"I wanted to follow up on the {opportunity_name} opportunity now that "
                f"it is in the {stage} stage.\n\n"
                f"I thought it would be helpful to connect and align on priorities, timing, "
                f"and the best next step for your team.\n\n"
                f"Would you be open to a quick conversation this week?\n\n"
                f"Best,\n"
                f"Justin"
            )
...
def draft_email(
    ...
    """
    Summary:
    draft_email is the main function used by the rest of the agent. If an
    OPENAI_API_KEY exists, the agent uses the LLM to draft the email. If there
    is no API key, it falls back to the template version so the project can
    still run locally during demos or grading.

    @param account_name: the account name for the email
    @param outreach_angle: the outreach strategy selected by rules.py
    @param evidence: the list of evidence facts selected by rules.py

    @return: an EmailDraft created by either the LLM or the fallback template
    """
