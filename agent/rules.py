...
# General number threshold, can be more accustomed after testing
...
# get_latest_product_usage returns the most recent product usage row.
# If there is no product usage data, it returns None.
# Using the max function returns the usage row with the biggest usage month.
...
# choose_outreach_angle chooses the best outreach angle using simple priority rules.
# The goal is to pick the strongest reason to contact the account right now.
...
# apply_rules runs the full rules step.
# It chooses the outreach angle and builds the supporting evidence.
